...
import { SignatureMap } from "./basic_types";
...
function createBaseSignedTransaction(): SignedTransaction {
  return { bodyBytes: new Uint8Array(), sigMap: undefined };
}
...
export const SignedTransaction = {
  encode(
    ...
    if (message.sigMap !== undefined) {
      SignatureMap.encode(message.sigMap, writer.uint32(18).fork()).ldelim();
    }
    return writer;
  },

  decode(input: _m0.Reader | Uint8Array, length?: number): SignedTransaction {
